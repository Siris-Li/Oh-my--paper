...
在启动任何后台 agent/Codex reviewer 之前，先检查当前工作目录是否是 git 仓库：

```bash
git rev-parse --is-inside-work-tree
```

如果不是 git 仓库，停止审查流程，不要调用 `/codex:rescue` 或 reviewer agent。提示用户进入具体论文项目目录并执行：
...
说明本地 git 不需要 push；它只是让 Claude Code 的 agent worktree 隔离机制工作。不要在 Desktop/Home 等大目录初始化 git。
